...
function makeValidComplete(overrides?: Partial<Complete>): Complete {
...
describe("Complete", () => {
  ...
  test("validates all reason types", () => {
    for (const reason of ["start", "timer", "end", "manual", "session", "counter"]) {
      const record = makeValidComplete({ reason: reason as any })
...
describe("AceTrace", () => {
  ...
  function makeRecords(count: number, baseDateMs = baseTimestamp.valueOf()): Complete[] {
...
describe("deserializeAceRecordTimestamp", () => {
  test("converts string timestamp to Date", () => {
    const record = makeValidComplete({ timestamp: "2025-01-15T10:00:00Z" as any })
    const deserialized = deserializeAceRecordTimestamp(record)
    expect(deserialized.timestamp).toBeInstanceOf(Date)
    expect((deserialized.timestamp as Date).toISOString()).toBe("2025-01-15T10:00:00.000Z")
  })
